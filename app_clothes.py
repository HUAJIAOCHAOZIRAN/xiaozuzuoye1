...
def get_client():
    # 优先使用页面输入的API配置
    api_key = st.session_state.get("deepseek_api_key", "") if "deepseek_api_key" in st.session_state else (DEEPSEEK_API_KEY if DEEPSEEK_API_KEY and DEEPSEEK_API_KEY != "your_deepseek_api_key_here" else "")
    base_url = st.session_state.get("deepseek_base_url", "") if "deepseek_base_url" in st.session_state else DEEPSEEK_BASE_URL

    if api_key and api_key != "your_deepseek_api_key_here":
        # 使用session_state存储客户端，避免重复创建
        client_key = f"{api_key}_{base_url}"
        if "api_client" not in st.session_state or st.session_state.get("api_client_key") != client_key:
            st.session_state.api_client = OpenAI(
                api_key=api_key,
                base_url=base_url
            )
            st.session_state.api_client_key = client_key
...
# API配置区域
st.markdown("## 🔧 API配置")
with st.expander("点击配置DeepSeek API（可选）", expanded=False):
    st.markdown("在此处配置DeepSeek API后，将使用AI智能识别颜色。如未配置，系统将使用备用机械式识别算法。")
    ...
    with col2:
        deepseek_base_url = st.text_input(
            "Base URL", 
            value=st.session_state.get("deepseek_base_url", "https://api.deepseek.com/v1"), 
    ...
    with col3:
        deepseek_model = st.text_input(
            "Model", 
            value=st.session_state.get("deepseek_model", "deepseek-chat"), 
    ...
    if st.button("保存API配置"):
        ...
        if "api_client_key" in st.session_state:
            del st.session_state.api_client_key
        st.success("✅ API配置已保存！")

# 显示API配置状态
has_api_config = (
    (st.session_state.get("deepseek_api_key", "") if "deepseek_api_key" in st.session_state else "") or 
    (DEEPSEEK_API_KEY if DEEPSEEK_API_KEY and DEEPSEEK_API_KEY != "your_deepseek_api_key_here" else "")
)
if has_api_config:
    st.success("✅ DeepSeek API已配置，将使用AI智能识别颜色")
else:
    st.warning("⚠️ DeepSeek API未配置，将使用备用机械式识别算法")
